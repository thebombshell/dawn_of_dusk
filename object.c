...
void dod_object_init(dod_object* t_object, dod_udword t_chunk_x, dod_udword t_chunk_y, dod_udword t_chunk_z, dod_single t_x, dod_single t_y, dod_single t_z) {
...
void dod_object_translate(dod_object* t_object, dod_single t_x, dod_single t_y, dod_single t_z) {
	...
	if (t_object->x < 0.0f || t_object->x >= 8.0f) {
		...
		t_object->chunk_x = (dod_udword)floorf(value / 8.0f);
		t_object->x = value - ((dod_single)chunk_x);
	...
	if (t_object->y < 0.0f || t_object->y >= 8.0f) {
		...
		t_object->chunk_y = (dod_udword)floorf(value / 8.0f);
		t_object->y = value - ((dod_single)chunk_y);
	...
	if (t_object->z < 0.0f || t_object->z >= 8.0f) {
		...
		t_object->chunk_z = (dod_udword)floorf(value / 8.0f);
		t_object->z = value - ((dod_single)chunk_z);
		t_object->info.flags.chunk_z_dirty = 1;
	}
}

void dod_object_teleport(dod_object* t_object, dod_udword t_chunk_x, dod_udword t_chunk_y, dod_udword t_chunk_z, dod_single t_x, dod_single t_y, dod_single t_z) {
...
void dod_object_rotate(dod_object* t_object, dod_single t_delta) {
	...
	t_object->rotation = fmodf(t_object->rotation + t_delta, M_PI * 2.0f);
...
void dod_object_set_rotation(dod_object* t_object, dod_single t_rotation) {
	...
	t_object->rotation = fmodf(t_rotation, M_PI * 2.0f);
	t_object->info.flags.rotation_dirty = 1;
}
